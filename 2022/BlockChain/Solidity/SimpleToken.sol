...
contract SimpleToken is ERC20Interface, OwnerHelper {
    ...
    function allowance(address owner, address spender) external view override returns (uint256) {
        return _allowances[owner][spender]; // 소유자, 보내는 사람(사용할 사람) 주소로 
    }

    function approve(address spender, uint amount) external virtual override returns (bool) {
        uint256 currentAllowance = _allowances[msg.sender][spender]; // 현재 사용할 수 있는 양
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external virtual override returns (bool) {
        _transfer(sender, recipient, amount); // 기본적인 보내기 함수
        emit Transfer(msg.sender, sender, recipient, amount); // 이번트 실행
        uint256 currentAllowance = _allowances[sender][msg.sender]; // 현재 보낸사람이 사용할 수 있는 양지정
        require(currentAllowance >= amount, "ERC20: transfer amount exceeds allowance");
        _approve(sender, msg.sender, currentAllowance, currentAllowance.sub(amount)); // 내 토큰을 사용할 수 있는 사람 지정
        return true;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint256 senderBalance = _balances[sender]; // 보낸사람의 잔고
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance");
        _balances[sender] = senderBalance.sub(amount); // 보낸만큼 차감
        _balances[recipient] += _balances[recipient].add(amount); // 받는사람은 추가
    }
